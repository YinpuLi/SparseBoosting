...
for pval in Pval[1]
    ...
    X, Y = GB18.test_data(500; p = pval, sigma = 1.0)
    X_test, Y_test = GB18.test_data(500; p = pval, sigma = 1.0)
    μ = GB18.fried(X)
    μ_test = GB18.fried(X_test)

    hypers = GB18.Hypers(X, Y, 1, 1, 0.95, 0.5)
    hypers.σ = sqrt(1)
    ...
    push!(HyperSigma, hypers.σ)
    println("opts.num_burn = $(opts.num_burn), σ = $(hypers.σ)")
end
...
for pval in Pval
    ...
    rng = Random.seed!(2019)
    ### simulate the training and testing data
    X, Y = GB18.test_data(500; p = pval, sigma = sqrt(10.0))
    X_test, Y_test = GB18.test_data(500; p = pval, sigma = sqrt(10.0))
    μ = GB18.fried(X)
    μ_test = GB18.fried(X_test)

    hypers = GB18.Hypers(X, Y, 1, 1, 0.95, 2.0)
    hypers.σ = sqrt(1)
    hypers.τ = 3 / 10 * var(Y) / gbdt.num_iterations
    hypers.α = 0.95
    hypers.s = ones(size(X, 2)) / size(X, 2)
    ...
    push!(HyperSigma_sigma10, hypers.σ)
end
