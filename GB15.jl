...
function _split!(
    ...
    candidate_features = sample(collect(non_zero_ind), wv, length(non_zero_ind); replace = false)
    ...
    @inbounds while (unsplittable || indf <= length(candidate_features)) && indf <= n_features
        ...
        while hi < n_samples
            ...
            if lo-1 >= min_samples_leaf && n_samples - (lo-1) >= min_samples_leaf
                ...
                purity = BayesRule4Split(nl, nr, lsum * lsum, rsum * rsum, hypers)
