...
# update: add get_count_of_var: the counts of splitting times on a predictor
        # add Dirichlet prior
...
function _fit(
    ...
    varcount = Array{Int}(undef, n_features) # to store the get_var_counts
...
function fit_tree(;
    ...
    root, indX, varcount = _fit(
        ...
        hypers,
        rng)
    return Tree{S}(root, indX, varcount)
end
