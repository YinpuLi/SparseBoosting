...
# update: TODO:changed the hypers part:
    # learning_rate = (hypers.σ / hypers.τ)²
...
# previous result: the varcount works fine, but the hypers.s does not:
   # when p is really large,the rmse boosted to large values
   # it bcz of that the algorithm will select the same feature to split on again and again
   # TODO: must fix this issue! the rmse is super large
   # need to check, not sure whether fixed or not
...
function _fit(
    ...
    # after running out of the stack, we finished buidling the tree
        # update the hypers.s
    # TODO: hypers.α needs to be updated
    shape_up = max.((hypers.varcount .+ hypers.α / length(hypers.s)), 1)
    hypers.s = rand(Dirichlet(shape_up))  # UpdateS!
        # returns a probability vector, length = p, the elements in the vector sum up to 1
