...
# update: change the _split! function:
    # TODO:     ~ change the _split!
            #   ~ when selecting the feature, penalize by the logsᵢ
            #   ~ ProbabilityWeights 那部分改了吧
            #   ~ 需要check一下 C++ version 的 gbm splitting rule

# update: change the hypers part:
    # TODO:learning_rate = (hypers.σ / hypers.τ)² 这个肯定是有问题的， 目前的设定得出来的是 learning_rate = 100, 结果导致rmse2 = Inf， 显然不对
...
function TBD_predict_and_rmse!(
    ...
    for l in collect(1 : opts.num_burn)
        ...
        if (opts.update_s) # true
            ...
            Store_s[l, :] = hypers.s
            #println("\nFor p = $(size(X, 2)), burn = $l,hypers.s = $(hypers.s).")
        end
...
function _split!(
    ...
    candidate_features = 1 : n_features#sample(collect(non_zero_ind), wv, length(non_zero_ind); replace = true)
    candidate_features = unique(candidate_features)

    # if the n_features = total_features = max_features = p; then non_constants_used = p

    @inbounds while (unsplittable || indf <= length(candidate_features)) && indf <= n_features
        # GB15: changed the indf <=non_constants_used

        ##### this is the origianl algorithm #####
        ##### I am changing it to the one with Dirichlet prior ####
        # simply deleted it and changed to the one with Dirichelt prior
        # shuffling algorithm
        #feature = let # uniformly pick up one index of features
        #        indr = rand(rng, indf:n_features)  # pick a randome element from the set of values in indf : n_features
        #        features[indf], features[indr] = features[indr], features[indf] # shuffle the array of feature index by Fisher-Yates shuffle algorithm
        #        features[indf] # then pick up
        #    end
        ##### end of the origianl algorithm ####

        #### Updated one with Dirichelt prior ####

        feature = candidate_features[indf]
        #### end of Updated one with Dirichlet prior ####

        ### END OF GB15 update  in _split! part ###
        rssq = tssq
        lssq = zero(U)
        rsum = tsum
        lsum = zero(U)
        ...
        while hi < n_samples
            ...
            if lo-1 >= min_samples_leaf && n_samples - (lo-1) >= min_samples_leaf
                ...
                purity = BayesRule4Split(nl, nr, lsum * lsum, rsum * rsum, hypers) .+ log(hypers.s[feature])
...
function _fit(
    ...
    @inbounds while length(stack) > 0
        ...
        #println("In the _fit func looping part, the hypers.varcount is updated, with value:\n$(hypers.varcount)")  # output the var counting numbers

    end

    #println("Current tree, the final var count vector is: \n$(hypers.varcount).")
    #println("Current tree, the updated hypers.s is: \n$(hypers.s)")
    return (root, indX)
end
...
function UpdateS!(hypers :: Hypers)
    ...
    # shape_up = max.((hypers.varcount .+ hypers.α / length(hypers.s)), 0.00001)
    # hypers.s = rand(Dirichlet(shape_up))# returns a probability vector, length = p, the elements in the vector sum up to 1
end
