...
# THIS version is not working properly, the well-written GB19.jl has lost
# I have re-written everything in GB100.jl. Check that version.

# Based on GB17.jl
# udpate: sample leaf parameter from N(Μ = ∑y/(σ²(1/τ + n/σ²)), Σ = 1/(1/τ + n / σ²))
    # added helper function:
        #getMu() to get Μ
        #getSigma() to get Σ
        #assisting _split!()
# add the slice sampling method: NOT OK
# TODO: using the slice sampling to UpdateAlpha!
    # NOT OK!
        # bcz some sᵢ's have been forced to 0,
        # log sᵢ = - Inf
        # In logLik part, messy result for psi



# TODO: update the hypers.α in the num_burn, together with UpdateS!
...
using MLBase, Roots, DataFrames, Roots
...
function TBD_predict_and_rmse!(
    ...
    for l in collect(1 : opts.num_burn)
        ...
        ### changed Apr 22
        cur_res = Y .- predict!(gbl, X)
        hypers_copy.σ_hat = GetSigma(X, Y)

        UpdateSigma!(hypers_copy, cur_res) # returns the updated hypers_copy.s
...
function _split!(
    ...
    node.label =  tsum / wsum # I have hashtaged this and changed it to:
    ...
    if (min_samples_leaf * 2 >  n_samples
     ...
     || tsum * node.label    > -1e-7 * wsum + tssq)
        ...
        node.is_leaf = true
        ###### udpate on Apr 22 #######
        Μ = getMu(tsum, hypers.σ, hypers.τ, n_samples)
        Σ = getSigma(hypers.σ, hypers.τ, n_samples)
        node.label = rand(Normal(Μ, Σ))
        ###### end on Apr 22 ######
        return
    end
    ...
    @inbounds if (unsplittable
        ...
        node.is_leaf = true
        #### udpate on Apr 22 #########
        # if sample no-split option, i.e, we have reaached to the leaf/ terminal node
        # added: sample leaf parameter from Normal Distribution
        # so that to udpate node.label, instead of node.label = tsum/wsum
        Μ = getMu(tsum, hypers.σ, hypers.τ, n_samples)
        Σ = getSigma(hypers.σ, hypers.τ, n_samples)

        node.label = rand(Normal(Μ, Σ))

        #### end of update on Apr 22 ##
        return
    else
...
function UpdateSigma!(hypers::Hypers, r::Vector{Float64})
    hypers.σ = update_sigma(r, hypers.σ_hat, hypers.σ)
end
...
function sliceSample(n, f , interval, w, hypers)
    ...
    for i in 1:n
        ...
        for j in seq
            if (fshift(hypers) < 0) != (f(hypers)<y)
                # signs don't macth, so we have a root
                root = find_zero(z -> f(hypers) - y, j)
                if j< root < j + w
                    push!(roots, root)
                end
            end
