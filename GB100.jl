...
# This version is a newly written version after files lost on May 8th
...
mutable struct Hypers   # this is more like a Hypers-and-helpers, because some of the filds are not hyper-parameters, but helpers
    ...
    λ           :: Union{Nothing, Float64, Int64}   # Tₘ ∼ exp decay(λ::Float64) or Poisson(λ::Int64)
    s           :: Vector{Float64}                  # probability vector
    varcount    :: Vector{Int64}                    # get_var_counts
    currentT    :: Int64                            # current Tₘ
    Hypers() = new()
...
function TBD_predict_and_rmse!(
    ...
    for l in collect(1 : opts.num_burn)
        ...
        # fit the model
        model = fit!(gbl, X, Y, hypers_copy, opts)
    ...
    gbl = GBLearner(gbdt, :regression)
    mymodel= fit!(gbl, X, Y, hypers, opts)
    # get prediction
    ...
    return rmse2#(rmse2, Store_s)

end

function TBD_predict_and_rmse2!(
    X                       :: Matrix{S},
    Y                       :: Vector{Float64},
    μ                       :: Vector{Float64},
    X_test                  :: Matrix{S},
    ...
    hypers.τ = 3 / 10 * var(trainY) / gbdt.num_iterations
    ...
    for l in collect(1 : opts.num_burn)
        ...
        # update sigma here
        model = fit!(gbl, trainX, trainY, hypers_copy, opts)
        UpdateSigma!(hypers_copy, testX, testY, testμ, gbl)
        if (opts.update_σ)
            hypers.σ = hypers_copy.σ
        end
    ...
    # fit the new model with the updated hypers.s
    gbl = GBLearner(gbdt, :regression)
    mymodel= fit!(gbl, trainX, trainY, hypers, opts)
    # get prediction
    mypred = predict!(gbl, X_test)
...
function _fit(
    ...
    stack   = NodeMeta{S}[root] # store the possible enternal nodes
    stack2  = NodeMeta{S}[]     # store the leaf nodes
    ...
    @inbounds while length(stack) > 0
        ...
        cur_T               = length(stack2)
        if opts.update_currentT
            hypers.currentT = cur_T
        end
...
function build_stump(labels::Vector{T}, features::Matrix{S}, hypers :: Hypers, opts :: Opts; rng = Random.GLOBAL_RNG) where {S, T <: Float64}
    return build_tree(labels, features, hypers, opts, 0, 1)
end
...
function fit_gb(gb :: GBAlgorithm, X, y, hypers, opts)
    stochastic_gradient_boost(gb, X, y, hypers, opts)
end
...
function fit!(gbl           :: GBLearner,
              ...
              gbl.model      = fit_gb(gbl.algorithm, instances, labels, hypers, opts)
end
...
function gbm(gbl            :: GBLearner,
             ...
             if cv_folds    == nothing
                 ...
                 for (i, train_inds) in enumerate(gen)
                     ...
                     gbl.model   = fit_gb(gbl.algorithm, trainX, trainY, hypers, opts)
...
function TunePara_CV(
    ...
    if hypers.λ != nothing
        println("hypers.λ is not nothing, but $(hypers.λ).")
    end
    cv_folds        = gbdt.cv_folds
    n, p            = size(x)

    hypers_temp     = deepcopy(hypers)
    hypers_temp2    = nothing
    cv_error_stack  = []
    mean_cv_err_stack = [] # storing the mean cv error

    push!(cv_error_stack, typemax(Float64))

    folds_indx      = collect(Kfold(n, cv_folds))

    for i in 1: length(param2update[:σ])

          println("Testing the case of $(param2update[:σ][i])")

          hypers_temp.σ = param2update[:σ][i]
          #println(hypers_temp)
          rmse_temp = []
          for l in 1 : length(folds_indx)
                ...
                hypers_temp.τ = 3 / 10 * var(y_CVtrain) / gbdt.num_iterations
                hypers_temp.s =  ones(size(x_CVtrain, 2)) / size(x_CVtrain, 2)
                hypers_temp.varcount = fill(0, size(hypers_temp.s, 1))
                hypers_temp.currentT = 0


                rmse2 = TBD_predict_and_rmse!(
                    x_CVtrain,
                    y_CVtrain,
                    x_CVtest,
                    y_CVtest,
          ...
          push!(mean_cv_err_stack, cv_error)
    ...
    println("The updated hyperparam combo is: σ = $(hypers_temp2.σ).")
    return (hypers_temp2, cv_error_stack[1], mean_cv_err_stack)
end
...
function deepcopy(hypers :: Hypers)
    ...
    newhypers.λ         = deepcopy(hypers.λ)
    newhypers.s         = deepcopy(hypers.s)
    newhypers.varcount  = deepcopy(hypers.varcount)
    newhypers.currentT  = deepcopy(hypers.currentT)
    return newhypers
end
...
function BayesRule4Split(nleft,
                         ...
                         if (isa(hypers.λ, Float64) && nright != 0 && nleft != 0) # the user has chosen the prior for Tₘ ∼ f ∝ Exp Decay(λ), and the process is splitting the node
                             ...
                             interResult = 0.5 * (logterm(nleft, hypers) + secondTerm(nleft, s2left, hypers)) +
                                           ...
                                           log(hypers.s[feature]) + hypers.λ * (T * log(T / (T + 1)) - log(T + 1))

                         elseif (isa(hypers.λ, Int64) && nright != 0 && nleft != 0)# the user has chosen the prior for Tₘ ∼ Poisson(λ::Int64), and the process is splitting the node
                             interResult = 0.5 * (logterm(nleft, hypers) + secondTerm(nleft, s2left, hypers)) +
                                           0.5 * (logterm(nright, hypers) + secondTerm(nright, s2right, hypers)) +
                                           log(hypers.s[feature]) + log(hypers.λ / (hypers.currentT + 1))

                         elseif (hypers.λ == nothing || nright == 0 || nleft == 0) # the user does not add any prior over Tₘ, or there are no more splitting(so no more addings on Tₘ)
                             interResult = 0.5 * (logterm(nleft, hypers) + secondTerm(nleft, s2left, hypers)) +
                                           0.5 * (logterm(nright, hypers) + secondTerm(nright, s2right, hypers)) +
                                           log(hypers.s[feature])
                         else # sth I did not consider properly
                             throw(error("Wrong!"))
                             return nothing
                         end
