...
# update: changed the Hyper part, deleting varcount from Hypers
    # the new tree/ in the new iteration, the updated hypers will be used;
    # but varcount will be re-initialized when building a new tree
    # ie. we will only keep using the hypers.s information, but not the varcount


# update: second trial for adding tree prior:
        # TODO:  add a tree prior of: the uniform distribution over available predictors for splitting rule assignment at each interior
        # TODO:  change the hypergeometric part
...
function _split!(
    ...
    @inbounds if (unsplittable
        ...
        varcount[best_feature] += 1 # update the count of selected split variable

    end
    println("In the _split! function, the varcount is \n$varcount")
    return varcount
end
...
function _fit(
    ...
    varcount = fill(0, n_features) # to store the get_var_counts
    ...
    @inbounds while length(stack) > 0
        ...
        if !node.is_leaf
            ...
            # udpate the hypers.s
            shape_up = varcount .+ hypers.α / size(hypers.s, 1)
            hypers.s = rand(Dirichlet(shape_up)) # returns a probability vector, length = p, the elements in the vector sum up to 1


            println("In the _fit func looping part, the varcount is \n$varcount")  # output the var counting numbers
        end
    end
