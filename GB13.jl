...
# udpate:
### in PART 0 ###
# added Opts: deciding the number of burning times
# added TBD_preduict_and_rmse! to fit and pred the final model

### in helper functions ###
# expanded deepcopy function for Opts and Hypers
# added UpdateS!: hypers.s should be updated after fitting the ensemble tree(sum of num_iterations trees)
# changed the UpdateS!(hyeprs) max.(~, 1 -> 0.01)

# update: change the hypers part:
    # TODO:learning_rate = (hypers.σ / hypers.τ)²
    # changed the shape_up formula based on fomula 25 to:
        # ~ shape_up = max.((hypers.varcount .+ hypers.α / length(hypers.s)), 1)
        # TODO: maybe the max.(sth, 1) the 1 is too large, I may think about change it to a smaller number
# previous result: the varcount works fine, but the hypers.s does not:
   # TODO:
   # need to check the s plot for different dimensions, not sure whether fixed or not
...
function UpdateS!(hypers :: Hypers)
    shape_up = max.((hypers.varcount .+ hypers.α / length(hypers.s)), 0.01)
