...
for pval in Pval
    ...
    X, Y = GB16.test_data(500; p = pval, sigma = 1.0)
    X_test, Y_test = GB16.test_data(500; p = pval, sigma = 1.0)
    μ = GB16.fried(X)
    μ_test = GB16.fried(X_test)

    hypers = GB16.Hypers(X, Y, 1, 1, 0.95, 0.5)
    hypers.σ = sqrt(1)
...
for pval in Pval
    ...
    rng = Random.seed!(2019)
    ### simulate the training and testing data
    X, Y = GB16.test_data(500; p = pval, sigma = sqrt(10.0))
    X_test, Y_test = GB16.test_data(500; p = pval, sigma = sqrt(10.0))
    μ = GB16.fried(X)
    μ_test = GB16.fried(X_test)

    hypers = GB16.Hypers(X, Y, 1, 1, 0.95, 2.0)
    hypers.σ = sqrt(1)
    hypers.τ = 3 / 10 * var(Y) / gbdt.num_iterations
    hypers.α = 0.95
    hypers.s = ones(size(X, 2)) / size(X, 2)
