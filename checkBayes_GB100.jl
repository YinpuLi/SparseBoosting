...
for pval in Pval
    ...
    x, y                    = GB100.test_data(500; p = pval, sigma = sqrt(1))
    ...
    hypers.α = 1
    hypers.λ = 2.0  # hypers.λ = 2 or 20 for Poisson(λ); hypers.λ = 1.0 or 2.0 for expDecay(λ = 1),vor hypers.λ = nothing for no priors
...
for pval in Pval
    ...
    X, Y                    = GB100.test_data(500; p = pval, sigma = sqrt(10.0))
